...
public:
	...
	RequestBuilder* addHeader(std::string key, std::string value);
	RequestBuilder* addBody(std::string body);
	std::string build();
private:
	HttpStatus::Code status;
	std::unordered_map<std::string, std::list<std::string>> request;
};
	namespace HttpStatus
	{
		//https://github.com/j-ulrich/http-status-codes-cpp/blob/main/HttpStatusCodes_C%2B%2B.h THANK U;
		enum Code
		{
			/*####### 1xx - Informational #######*/
